...
static float _erf(float x)
{
    ...
    const float y = 1.0 - (((((ERF_A5 * t + ERF_A4) * t) + ERF_A3) * t + ERF_A2) * t + ERF_A1) * t * epow;
    return sign * y;
}
...
static float _erfc(const float x) // we don't need this yet
{
    ...
    m_absf(x, &z);
    t = 1.0 / (1.0 + 0.5 * z);
    m_pow(M_E, -z * z - ERFC_A1 + t * (ERFC_A2 + t * (ERFC_A3 + t * (ERFC_A4 + t * (ERFC_A5 + t * (ERFC_A6 + t * (ERFC_A7 + t * (ERFC_A8 + t * (ERFC_A9 + t * ERFC_A10)))))))), &epow);
...
int m_normal_cdf(const float x, const float mean, const float std, float *result) // normalcdf(-1E99, x, mean, std)
{
...
static const float IDP_C0 = 2.515517, IDP_C1 = 0.802853,
    IDP_C2 = 0.010328, IDP_D0 = 1.432788,
    IDP_D1 = 0.189269, IDP_D2 = 0.001308;

static float _inverse_dist_approx(const float t)
{
    return t - ((IDP_C2 * t + IDP_C1) * t + IDP_C0) / (((IDP_D2 * t + IDP_D1) * t + IDP_D0) * t + 1.0);
}
