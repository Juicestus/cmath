/// @: Justus Languell <jlanguell@extendable.co>

#include "cmath.h"

// This is actually a pretty shitty approximate
// Accurate to only ~3 decimals
static double _arcsin_abramowitz_aprox(const double x)
{
  if (x < 0) return -_arcsin_abramowitz_aprox(-x);
  float r_sqrt;
  if (m_sqrt(&r_sqrt, 1. - x)) return 1;
  return M_PI_2 - r_sqrt * (1.5707288 - 0.2121144 * x + 0.0742610 * x * x -
                            0.0187293 * x * x * x);
}

int m_asin(float* result, const float ratio)
{
  if (-1 > ratio || ratio > 1) return 1;
  *result = _arcsin_abramowitz_aprox(ratio);
  return 0;
}

int m_acos(float* result, const float ratio)
{
  float r_asin;
  m_asin(&r_asin, ratio);
  *result = M_PI_2 - r_asin;
  return 0;
}

int m_atan(float* result, const float ratio)
{
  if (ratio == 0) return 1;
  const float radicand = 1. / ((ratio * ratio) + 1);
  float r_sqrt, r_acos, r_signf;
  if (m_sqrt(&r_sqrt, radicand)) return 1;
  if (m_acos(&r_acos, r_sqrt)) return 1;
  if (m_signf(&r_signf, ratio)) return 1;
  *result = r_acos * r_signf;
  return 0;
}